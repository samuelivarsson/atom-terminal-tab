...
export default class TerminalSession {
  ...
  constructor(editor, config = {}) {
    ...
    this.isOpen = true;
    this.test = 0;
    if (this.editor == null || this.editor.buffer == null) {
      this.filePath = null;
    } else {
      this.filePath = this.editor.buffer.getPath();
    }
    this.config = config;
  ...
  insertCustomText(customText) {
    if (this.filePath == null || !this.editor.buffer) {
      return;
    }
    if (atom.config.get('iv-terminal.saveOnInsert')) {
      this.editor.save();
    }

    filePath = this.filePath;
    text = `${customText
      .replace(/\$F/, path.basename(filePath ? filePath : '.'))
      .replace(/\$D/, path.dirname(filePath ? filePath : '.'))
      .replace(/\$\$/, '$')}`
    newLine = "\n";
  ...
  _getWorkingDirectory() {
    ...
    if (this.editor != null && this.filePath != null) {
